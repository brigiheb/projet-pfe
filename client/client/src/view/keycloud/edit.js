...
export default function EditKeyCloud() {
    const { id } = useParams()
    ...
    useEffect(() => {
        const getDataById = async () => {
            const { data } = await axios.get(`http://localhost:5000/keycloudGet/${id}`)
    ...
    const putData = async (e) => {
        ...
        await axios.put(`http://localhost:5000/keycloudUp/${id}`, Data)
        navigate('/Tkeycloud')
    }
